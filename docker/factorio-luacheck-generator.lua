...
do -- Decode the api file check for local cache first
	...
	lfs.mkdir(".io")
	local api_url = "https://lua-api.factorio.com/" .. factorio_version .. "/runtime-api.json"
	local api_fn = ".io/runtime-api.json"
	local api_file = io.open(api_fn, "r")
	if not api_file then
		local api_from_web = assert(https.request(api_url))
		api_file = assert(io.open(api_fn, "w"))
		api_file:write(api_from_web)
		api_file:flush()
		api_file = io.open(api_fn, "r")
	end
